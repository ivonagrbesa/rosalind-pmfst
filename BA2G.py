...
def RandomKmers(dna, k):
  motifs=[]
  for linija in dna:
    r=random.randrange(0, len(linija)-k+1) #zadnji el nije ukljucen
...
def Score(motifs):
  score=0
  for i in range(len(motifs[0])): #idemo od 0 do k (po duljini svakog motiva)
    j=[motif[i] for motif in motifs] #svaki motiv rastavimo na niz od k slova
    score+=len(j)-max(j.count("A"), j.count("C"), j.count("G"), j.count("T")) #izbrojimo sva velika slova tj kojih ima najvise => k minus taj br se dodaje scoreu
  return score
...
def GibbsSamplerAtom(dna, k, t, N):
  ...
  for j in range(1, N):
    ...
    najvjer=BestPattern(dna[i], k, profile) #i s obz na tu matricu izaberemo novi kmer iz i-te linije
...
def GibbsSampler (dna, k, t, N, rep=20):
  bestmotifs=GibbsSamplerAtom(dna, k, t, N) #napravi gibbsovo sampliranje od N puta ka pocetno
